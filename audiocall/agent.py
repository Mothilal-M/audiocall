...
def _role_context(role: str) -> str:
    ...
    if role == "worker":
        return (
            ...
            "LANGUAGE: Start the call in HINDI (the worker is most comfortable in "
            "Hindi). Open in Hindi and keep speaking Hindi unless the worker "
            "switches to another language, in which case follow them.\n\n"
            "1. Open with a SHORT greeting in HINDI that confirms the person:\n"
            f"   - 'Namaste {WORKER_NAME} ji, main {PROJECT_NAME} project ki team "
            "se baat kar raha hoon. Aaj site ka kaam kaisa chal raha hai, do "
            "minute baat kar sakte hain?'\n\n"
    ...
    return (
        ...
        "1. Open with a SHORT greeting that confirms the person:\n"
        f"   - 'Hi {MANAGER_NAME}, calling from the project team about "
        f"{PROJECT_NAME}. Got a few minutes to go over the site status?'\n\n"
        ...
        "       • Site clearing, excavation & foundation for all 4 towers — done\n"
        "       • Tower A: full structure (all 12 floors) cast & cured — done\n"
        "       • Tower B: structure up to 8th floor — done\n"
        "       • Tower C & D: up to 4th floor structure — done\n"
        "       • Internal brickwork & plastering for Tower A floors 1–6 — done\n"
        "       • Underground water tank & sewerage lines — done\n"
        "       • Boundary wall & main site access road — done\n"
